"""
E2E performance tests for file transfer.

Tests the full stack (nginx -> backend -> file serving) with various file sizes.
"""
import os
import sys
...
def test_files():
    ...
    if not TEST_DATA_DIR.exists():
        pytest.skip("Test data directory not found. Run generate_test_files.py first.")
...
class TestBaselinePerformance:
    """Test baseline performance with current setup (StaticFiles + nginx buffering)."""

    def test_small_files_1kb_to_100kb(self, tester, test_files):
        """Test performance for small files (1KB-100KB)."""
        small_files = [
            f
            for f in test_files
        ...
        results = tester.run_multiple_tests(small_files, runs=3)
        ...
        for filename, metrics_list in results.items():
            ...
            # Small files should transfer quickly (< 1 second)
    ...
    def test_medium_files_500kb_to_1mb(self, tester, test_files):
        """Test performance for medium files (500KB-1MB)."""
        medium_files = [f for f in test_files if "500kb" in f or "1mb" in f]

        if not medium_files:
            pytest.skip("No medium test files found")

        results = tester.run_multiple_tests(medium_files, runs=3)

        for filename, metrics_list in results.items():
            stats = tester.calculate_statistics(metrics_list)
            assert stats["success_rate"] == 1.0, f"{filename} had failures"

            # Medium files should transfer in reasonable time (< 5 seconds)
            avg_time = stats["avg_time"]
    ...
    def test_large_files_5mb_to_10mb(self, tester, test_files):
        """Test performance for large files (5MB-10MB)."""
        large_files = [f for f in test_files if "5mb" in f or "10mb" in f]

        if not large_files:
            pytest.skip("No large test files found")

        results = tester.run_multiple_tests(large_files, runs=3)

        for filename, metrics_list in results.items():
            stats = tester.calculate_statistics(metrics_list)
            assert stats["success_rate"] == 1.0, f"{filename} had failures"

            # Large files should complete (< 30 seconds)
            avg_time = stats["avg_time"]
    ...
    @pytest.mark.slow
    def test_xlarge_files_25mb_to_50mb(self, tester, test_files):
        """Test performance for extra large files (25MB-50MB+)."""
        xlarge_files = [f for f in test_files if "25mb" in f or "50mb" in f]

        if not xlarge_files:
            pytest.skip("No extra large test files found")

        results = tester.run_multiple_tests(xlarge_files, runs=2)
        ...
        for filename, metrics_list in results.items():
            ...
            # Extra large files should complete (< 60 seconds)
    ...
    @pytest.mark.slow
    def test_save_baseline_results(self, tester, test_files):
        """Run comprehensive tests and save baseline results."""
        RESULTS_DIR.mkdir(parents=True, exist_ok=True)

        print("\n\n=== Running Comprehensive Baseline Tests ===\n")

        all_results = tester.run_multiple_tests(test_files, runs=3)
        ...
        with open(results_file, "w") as f:
            ...
            f.write("Configuration: StaticFiles + nginx proxy_buffering on\n\n")
