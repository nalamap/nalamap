...
test("check if stores are exposed", async ({ page }) => {
  ...
  await page.waitForFunction(
    () => {
      const hasStores =
        !!(window as any).useLayerStore || !!(window as any).useMapStore;
  ...
  const hasStoreLog = consoleMessages.some((msg) =>
    msg.includes("Stores exposed"),
  );
  ...
  const windowCheck = await page.evaluate(() => {
    return {
      ...
      hasUseSettingsStore:
        typeof (window as any).useSettingsStore !== "undefined",
