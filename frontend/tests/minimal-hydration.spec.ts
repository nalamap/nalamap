...
test("minimal hydration check", async ({ page }) => {
  ...
  const pageStructure = await page.evaluate(() => {
    return {
      ...
      nextDataKeys: (window as any).__NEXT_DATA__
        ? Object.keys((window as any).__NEXT_DATA__)
        : [],
    };
  });
  ...
  const storesCheck = await page.evaluate(() => {
    return {
      ...
      hasDataStoresExposedAttr:
        document.body?.getAttribute("data-stores-exposed") === "true",
    };
  });
  ...
  console.log(`Test Script Ran: ${testScriptRan ? "✅" : "❌"}`);
  console.log(
    `Next.js Hydrated (__NEXT_DATA__): ${pageStructure.hasNextData ? "✅" : "❌"}`,
  );
  console.log(
    `Stores Exposed (flag): ${storesCheck.hasStoresExposedFlag ? "✅" : "❌"}`,
  );
  console.log(
    `Stores Exposed (DOM attr): ${storesCheck.hasDataStoresExposedAttr ? "✅" : "❌"}`,
  );
  console.log(
    `Has useLayerStore: ${storesCheck.hasUseLayerStore ? "✅" : "❌"}`,
  );
