...
test.describe('GeoServer backend preload - Advanced scenarios', () => {
  ...
  test('handles partial import when some backends fail to preload', async ({ page }) => {
    ...
    await expect(page.getByText(/Failed to preload.*fail\.example/)).toBeVisible();

    // Successful backends should be visible
    await expect(page.getByRole('listitem').filter({ hasText: 'Success Backend' })).toBeVisible();
    await expect(page.getByRole('listitem').filter({ hasText: 'Another Success' })).toBeVisible();
  });

  test('handles import of settings without model_settings gracefully', async ({ page }) => {
    ...
    await expect(page.getByText('Prefetched 1 imported backend successfully.')).toBeVisible();
  ...
  test('prevents multiple concurrent imports', async ({ page }) => {
    ...
    await expect(page.getByText(/Prefetched.*successfully/)).toBeVisible({ timeout: 5000 });
  ...
  test('validates backend URL before prefetching', async ({ page }) => {
    ...
    await expect(page.getByText(/Failed to preload|Please provide a GeoServer URL/i)).toBeVisible();
  });

  test('preserves existing backends when importing additional ones', async ({ page }) => {
    ...
    await page.getByPlaceholder('GeoServer URL').fill('https://existing.example/geoserver');
    await page.getByPlaceholder('Name (optional)', { exact: true }).fill('Existing Backend');
    ...
    await expect(page.getByRole('listitem').filter({ hasText: 'Existing Backend' })).toBeVisible();
    ...
    await expect(page.getByRole('listitem').filter({ hasText: 'Imported Backend' })).toBeVisible();
  });
  ...
  test('handles network timeout gracefully', async ({ page }) => {
    ...
    await expect(page.getByText(/Settings imported.*Unable to preload/i)).toBeVisible();
  });
});
