...
const mockHospitalPointsLayer = {
  ...
  llm_description: '3 hospitals (points) found matching amenity=hospital near Brazil',
  score: 0.85,
...
const mockHospitalAreasLayer = {
  ...
  llm_description: '1 hospital (area) found matching amenity=hospital near Brazil',
  score: 0.85,
  bounding_box: [-43.1800, -22.9100, -43.1790, -22.9090],
  visible: true,
  properties: {
    feature_count: 1,
    query_amenity_key: 'Hospital',
...
async function countRenderedFeatures(page: Page): Promise<number> {
  ...
  const svgCount = await page.locator('.leaflet-overlay-pane path, .leaflet-overlay-pane circle').count();
...
test.describe('Agent Chat Interface - Add Layer Button', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.route('**/uploads/test_hospital_points_brazil.geojson', async (route) => {
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
    ...
    await page.route('**/uploads/test_hospital_areas_brazil.geojson', async (route) => {
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
  ...
  test('BUG REPRODUCTION: Add to Map button - layers should render immediately', async ({ page }) => {
    /**
    ...
    const mockChatResponse = {
      messages: [
        ...
        { 
          type: 'ai', 
          content: 'I found 2 layers with hospitals in Brazil: 3 point features and 1 area feature.',
        },
      ],
    ...
    await expect(page.getByText('I found 2 layers')).toBeVisible({ timeout: 5000 });
    ...
    await expect(page.getByText('Hospitals (Points) near Brazil')).toBeVisible();
    ...
    console.log(`Total rendered features after adding both layers: ${totalFeatures}`);
    
    expect(totalFeatures).toBeGreaterThanOrEqual(4); // All features should be visible
  });

  test('VERIFICATION: Toggle visibility works correctly after adding layer', async ({ page }) => {
    /**
  ...
  test('EDGE CASE: Multiple rapid Add to Map clicks should not cause issues', async ({ page }) => {
    /**
