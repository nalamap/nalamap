...
test.describe("GeoServer backend preload", () => {
  ...
  test("shows an error when the backend cannot be prefetched", async ({
    page,
  }) => {
    ...
    await page
      ...
      .fill("https://example.com/geoserver");
  ...
  test("displays a success message and resets inputs after prefetching", async ({
    page,
  }) => {
    await page.route("**/settings/geoserver/preload", async (route) => {
      await route.fulfill({
    ...
    await expect(
      page.getByRole("listitem").filter({ hasText: "Demo Geo" }),
    ).toBeVisible();
  });

  test("exports the current settings snapshot including prefetched backends", async ({
    page,
  }) => {
    await page.route("**/settings/geoserver/preload", async (route) => {
      await route.fulfill({
  ...
  test("imports settings, prefetches configured backends, and ignores file session id", async ({
    page,
  }) => {
    ...
    await expect(
      page.getByRole("listitem").filter({ hasText: "Geo One" }),
    ).toBeVisible();
    await expect(
      page.getByRole("listitem").filter({ hasText: "Geo Two" }),
    ).toBeVisible();

    const geoOneItem = page
      .getByRole("listitem")
      .filter({ hasText: "Geo One" });
    const geoTwoItem = page
      .getByRole("listitem")
