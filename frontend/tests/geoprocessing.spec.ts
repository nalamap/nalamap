...
async function setupMocks(page: Page) {
  ...
  await page.route('**/*.blob.core.windows.net/data/*hospitals*.json*', (route) => {
    route.fulfill({
      status: 200,
      contentType: 'application/json',
      body: JSON.stringify(hospitalPointsGeoJSON),
  ...
  await page.route('**/*.blob.core.windows.net/data/*buffer*.json*', (route) => {
    route.fulfill({
      status: 200,
      contentType: 'application/json',
      body: JSON.stringify(hospitalBufferGeoJSON),
...
test.describe('Geoprocessing Operations', () => {
  ...
  test('should add a layer and render it on the map', async ({ page }) => {
    ...
    const featureCount = await page.locator('.leaflet-overlay-pane path, .leaflet-overlay-pane circle, .leaflet-marker-pane img, .leaflet-overlay-pane canvas').count();
    console.log('Feature count on map:', featureCount);
    expect(featureCount).toBeGreaterThan(0);
  });

  test('should perform buffer operation and display result', async ({ page }) => {
    ...
    expect(layersInStore.some((l: any) => l.id === 'hospitals-for-buffer')).toBe(true);
    expect(layersInStore.some((l: any) => l.id === 'hospitals-buffer-100km')).toBe(true);
    ...
    const featureCount = await page.locator('.leaflet-overlay-pane path, .leaflet-overlay-pane polygon').count();
  ...
  test('should support Azure Blob Storage URLs with SAS tokens', async ({ page }) => {
    ...
    const azureBlobLayer = {
      ...
      layer_type: 'GEOJSON',
      data_link: 'https://stnalamapdev.blob.core.windows.net/data/hospitals_abc123.json?sv=2021-06-08&se=2025-10-06T10%3A30%3A00Z&sr=b&sp=r&sig=test',
      visible: true,
    };
    ...
    const featureCount = await page.locator('.leaflet-overlay-pane path, .leaflet-overlay-pane circle, .leaflet-marker-pane img').count();
    console.log('Azure Blob layer feature count:', featureCount);
    expect(featureCount).toBeGreaterThan(0);
  });

  test('should handle geoprocessing result with multiple features', async ({ page }) => {
