    ...
    try:
        ...
        for layer_dict in request.layers:
            try:
                ...
                logger.warning(f"Could not convert layer to GeoDataObject: {e}")
                continue
        ...
        for layer in geodata_layers:
            needs_styling = (
                not hasattr(layer, 'style') or 
                layer.style is None or
        ...
        if not layers_needing_styling:
            ...
            styled_layer_dicts = [layer.dict() if hasattr(layer, 'dict') else layer.__dict__ for layer in geodata_layers]
            return AutoStyleResponse(
                success=True,
        ...
        styling_prompt = (
            ...
            f"{', '.join(layer_names)}. Please check for layers needing styling using check_and_auto_style_layers, "
            f"then analyze the layer names using auto_style_new_layers, and finally apply intelligent "
            f"AI-powered styling to these layers using style_map_layers with appropriate cartographic colors."
        )
        ...
        logger.info(f"Successfully applied automatic styling to {len(layers_needing_styling)} layers")
        ...
        logger.error(f"Error in auto_style_layers: {str(e)}")
        return AutoStyleResponse(
            success=False,
            message=f"Error applying automatic styling: {str(e)}",
