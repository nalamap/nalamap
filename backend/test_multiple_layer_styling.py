...
def test_multiple_layer_styling():
    ...
    state["messages"] = [
        HumanMessage(
            content="Apply 3 different warm colors to the available layers - each layer should get a different warm color that you think is appropriate"
        )
    ]
    ...
    print("Initial layers: {len(state['geodata_layers'])}")
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print(
        "1. ✅ Fixed concurrency issue with Annotated[List[GeoDataObject], update_geodata_layers]"
    )
