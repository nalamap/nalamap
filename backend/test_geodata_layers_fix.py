...
def test_geodata_layers_reducer():
    ...
    print(f"Current layers count: {len(current_layers)}")
    print(f"New layers count: {len(new_layers)}")
    print(f"Result layers count: {len(result)}")
...
def test_agent_state_basic():
    ...
    try:
        ...
        print(f"geodata_layers type: {type(state.geodata_layers)}")
        print(f"geodata_layers value: {state.geodata_layers}")
        
        print("✅ GeoDataAgentState works correctly!")
        return True
    except Exception as e:
        print(f"❌ GeoDataAgentState failed: {e}")
        return False
...
if __name__ == "__main__":
    ...
    print()
    print("🎉 All tests passed! The concurrency fix should resolve the multiple layer styling issue.")
    print()
    print("The key changes made:")
    print("1. Added 'update_geodata_layers' reducer function to handle concurrent updates")
    print("2. Changed 'geodata_layers' field to use Annotated type with the reducer")
    print("3. This allows multiple tools to update geodata_layers simultaneously without conflicts")
