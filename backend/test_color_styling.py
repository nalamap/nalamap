...
from services.tools.styling_tools import normalize_color, style_map_layers
...
def test_color_styling():
    ...
    print("coral -> {normalize_color('coral')}")
    print("peach -> {normalize_color('peach')}")
    print("brown -> {normalize_color('brown')}")
    print("darkorange -> {normalize_color('darkorange')}")

    print("\n" + "=" * 50)
    print("Color styling is ready with:")
    print(
        "✅ Added comprehensive logging to track layer matching and color application"
    )
    ...
    print("\nKey improvements:")
    print(
        "1. Colors like 'coral', 'peach', 'brown' are now converted to proper hex values"
    )
    ...
    print(
        "- Verify that the agent uses exact layer names like 'city.geojson' and 'forrest.geojson'"
    )
