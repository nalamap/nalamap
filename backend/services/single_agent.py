...
def create_geo_agent() -> CompiledGraph:
    ...
    system_prompt = (
        ...
        "- WHEN USERS ASK ABOUT A DATASET: You MUST access both 'geodata_last_results' and 'geodata_layers' arrays directly. These are arrays of GeoDataObject that you can access directly in your context. DO NOT say you don't have access - you DO have direct access to these objects.\n"
        "- HOW TO FIND THE DATASET: Search for a matching title in both arrays. For example, if the user mentions 'Population Density', search for objects where title.includes('Population Density') or name.includes('Population Density').\n"
        "- IF FOUND: Always extract and use ALL available metadata fields: title, name, description, llm_description, data_source, data_origin, layer_type, data_type, bounding_box, and any other available properties.\n"
        "- IF NOT FOUND EXACTLY: Use fuzzy matching to find similar datasets in the arrays by searching for partial matches in titles or names.\n"
        "- RESPONSE FORMAT: Always start responses with factual information directly from the dataset metadata, THEN add explanatory context if needed.\n"
        ...
        "- DATASET QUESTIONS: When users ask about a specific dataset like 'Tell me more about this dataset', 'What does the Population Density dataset show?', or similar:\n"
        "  1. IMMEDIATELY search both 'geodata_last_results' and 'geodata_layers' arrays for matching datasets\n"
        "  2. If user recently added a layer to map, PRIORITIZE looking in geodata_layers for recently added datasets\n"
        "  3. Use EXACT information from the dataset properties and metadata in your response, not generic information\n"
        "  4. INCLUDE source attribution, data type, coverage area, and resolution when available\n\n"
        "# DATA HANDLING\n"
