...
async def apply_automatic_styling_to_new_layers(geodata_layers: List[GeoDataObject]) -> List[GeoDataObject]:
    """
    ...
    styling_prompt = (
        ...
        f"{', '.join(layer_names)}. Please apply intelligent AI-powered styling to these layers based on their names. "
        f"Use your best judgment to determine appropriate cartographic colors for each layer."
    )
...
def trigger_automatic_styling_for_upload(layer_name: str, geodata_layers: List[GeoDataObject]) -> List[GeoDataObject]:
    """
    ...
    logger.info(f"Layer '{layer_name}' uploaded - automatic styling will be triggered on next chat interaction")
    return geodata_layers
